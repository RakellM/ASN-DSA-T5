...
numerical_variables.remove("Retained")
...
my_pipeline = ColumnTransformer([
    ("cat", step_categorical, categorical_variables),
    ("num", step_numerical, numerical_variables),
    ("bool", bool_pipeline, boolean_variables)
])

# %% 

## Applying the preprocessing pipeline to training and test data
X_train_tranformed = my_pipeline.fit_transform(X_train)
...
grid_lr = GridSearchCV(modelo_lr, param_grid_lr, cv=5, scoring='f1', n_jobs=-1)
grid_lr.fit(X_train_tranformed, y_train)
melhor_lr = grid_lr.best_estimator_
print("Melhor parâmetro LR:", grid_lr.best_params_)

# Random Forest
modelo_rf = RandomForestClassifier(random_state=42)
param_grid_rf = {'n_estimators': [100, 200], 'max_depth': [5, 10, None]}
grid_rf = GridSearchCV(modelo_rf, param_grid_rf, cv=5, scoring='f1', n_jobs=-1)
grid_rf.fit(X_train_tranformed, y_train)
melhor_rf = grid_rf.best_estimator_
print("Melhor parâmetro RF:", grid_rf.best_params_)

# XGBoost
modelo_xgb = XGBClassifier(use_label_encoder=False, eval_metric='logloss', random_state=42)
param_grid_xgb = {'n_estimators': [100, 300], 'max_depth': [3, 5], 'learning_rate': [0.01, 0.1]}
grid_xgb = GridSearchCV(modelo_xgb, param_grid_xgb, cv=5, scoring='f1', n_jobs=-1)
grid_xgb.fit(X_train_tranformed, y_train)
melhor_xgb = grid_xgb.best_estimator_
print("Melhor parâmetro XGB:", grid_xgb.best_params_)

# %%

# Dictionary of best models for evaluation
models = {
...
for name, model in models.items():
    y_pred_train = model.predict(X_train_tranformed)
    y_pred_test = model.predict(X_test_transformed)
    results.append({
        "model": name,
        "F1_Treino": f1_score(y_train, y_pred_train),
...
# Summary comparison in a DataFrame
df_results = pd.DataFrame(results)
print(df_results)

# %%

## Choose model to analyze threshold
model_escolhido = melhor_lr
y_proba = model_escolhido.predict_proba(X_test_transformed)[:, 1]
...
for limiar in limiares:
    ...
    precisions.append(precision_score(y_test, y_pred_limiar))
    recalls.append(recall_score(y_test, y_pred_limiar))

# Plot Precision vs Recall by threshold
plt.figure(figsize=(8, 5))
plt.plot(limiares, precisions, label='Precisão', marker='o')
plt.plot(limiares, recalls, label='Recall', marker='x')
plt.xlabel("Limiar de Classificação")
plt.ylabel("Métrica")
plt.title("Precision vs Recall - Regressão Logística")
plt.grid(True)
plt.legend()
plt.show()
...
## Create final pipeline including preprocessing and final model# 
...
joblib.dump(model_final, "modelo_final_retencao.joblib")
print("Modelo final salvo como 'modelo_final_retencao.joblib'.")
